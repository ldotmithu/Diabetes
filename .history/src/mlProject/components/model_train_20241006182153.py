...
class ModelTrain:
    ...
    def train(self):
        ...
        feature_selector = SelectFromModel(LogisticRegression(C=0.01))
        ...
        pred = best_model.predict(X_test_selected)
        logging.info(classification_report(y_test, pred))
        logging.info("ROC AUC Score: {}".format(roc_auc_score(y_test, best_model.predict_proba(X_test_selected)[:, 1])))
